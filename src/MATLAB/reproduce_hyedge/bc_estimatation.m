...
for index = 1:graph_num
    ...
    beacons = [];
    while length(beacons) < beacons_num
        beacon = floor(rand()*num_nodes) + 1;
    ...
    sample_size = all_pairs_hyedges;
    fprintf('hyedge sample size is %d\n' ,ceil(sample_size));
    % get the target nodes to estimate BC
    target_nodes = get_targetnodes(G, target_k, ceil(sample_size), all_nodes, p_nodes);
    real_bc = normalized_wbc(target_nodes)';
    num_pairs = ceil(2*log(2*(num_nodes^3))./eps^2);
